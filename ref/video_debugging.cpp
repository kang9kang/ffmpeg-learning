...
void print_timing(char *name, AVFormatContext *avf, AVCodecContext *avc, AVStream *avs)
{
    ...
    if (avf != NULL)
    {
        logging("\t\tstart_time=%d duration=%d bit_rate=%d start_time_realtime=%d", avf->start_time, avf->duration,
                avf->bit_rate, avf->start_time_realtime);
    }
    else
    {
        logging("\t\t->NULL");
    ...
    if (avc != NULL)
    {
        logging("\t\tbit_rate=%d ticks_per_frame=%d width=%d height=%d gop_size=%d keyint_min=%d sample_rate=%d "
                "profile=%d level=%d ",
